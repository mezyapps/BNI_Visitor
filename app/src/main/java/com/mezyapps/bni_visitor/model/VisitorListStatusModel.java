package com.mezyapps.bni_visitor.model;

import android.os.Parcel;
import android.os.Parcelable;

public class VisitorListStatusModel implements Parcelable {

    String visitor_id;
    String name;
    String mobile_no;
    String email_id;
    String category;
    String location;
    String source;
    String person_name;
    String status;
    String chapter_name;
    String launch_dc;
    String description;
    String Follow_up_date_time;
    String inserted_date_time;

    public String getInserted_date_time() {
        return inserted_date_time;
    }

    public void setInserted_date_time(String inserted_date_time) {
        this.inserted_date_time = inserted_date_time;
    }

    public VisitorListStatusModel() {
    }


    public static  final Parcelable.Creator CREATOR=new Parcelable.Creator()
    {


        @Override
        public VisitorListStatusModel createFromParcel(Parcel source) {
            return new VisitorListStatusModel(source);
        }

        @Override
        public VisitorListStatusModel[] newArray(int size) {
            return new VisitorListStatusModel[0];
        }
    };

    public VisitorListStatusModel(Parcel source) {
        this.visitor_id=source.readString();
        this.name = source.readString();
        this.mobile_no = source.readString();
        this.email_id = source.readString();
        this.category = source.readString();
        this.location = source.readString();
        this.mobile_no = source.readString();
        this.source = source.readString();
        this.person_name = source.readString();
        this.status = source.readString();
        this.description = source.readString();
        this.chapter_name = source.readString();
        this.launch_dc = source.readString();
        this.description = source.readString();
        this.Follow_up_date_time = source.readString();

    }

    @Override
    public int describeContents() {
        return 0;
    }

    @Override
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeString(this.visitor_id);
        dest.writeString(this.name);
        dest.writeString(this.mobile_no);
        dest.writeString(this.email_id);
        dest.writeString(this.category);
        dest.writeString(this.location);
        dest.writeString(this.mobile_no);
        dest.writeString(this.source);
        dest.writeString(this.person_name);
        dest.writeString(this.status);
        dest.writeString(this.description);
        dest.writeString(this.chapter_name);
        dest.writeString(this.launch_dc);
        dest.writeString(this.description);
        dest.writeString(this.Follow_up_date_time);

    }


    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description;
    }

    public String getFollow_up_date_time() {
        return Follow_up_date_time;
    }

    public void setFollow_up_date_time(String follow_up_date_time) {
        Follow_up_date_time = follow_up_date_time;
    }




    public String getVisitor_id() {
        return visitor_id;
    }

    public void setVisitor_id(String visitor_id) {
        this.visitor_id = visitor_id;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getMobile_no() {
        return mobile_no;
    }

    public void setMobile_no(String mobile_no) {
        this.mobile_no = mobile_no;
    }

    public String getEmail_id() {
        return email_id;
    }

    public void setEmail_id(String email_id) {
        this.email_id = email_id;
    }

    public String getCategory() {
        return category;
    }

    public void setCategory(String category) {
        this.category = category;
    }

    public String getLocation() {
        return location;
    }

    public void setLocation(String location) {
        this.location = location;
    }

    public String getSource() {
        return source;
    }

    public void setSource(String source) {
        this.source = source;
    }

    public String getPerson_name() {
        return person_name;
    }

    public void setPerson_name(String person_name) {
        this.person_name = person_name;
    }

    public String getStatus() {
        return status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public String getChapter_name() {
        return chapter_name;
    }

    public void setChapter_name(String chapter_name) {
        this.chapter_name = chapter_name;
    }

    public String getLaunch_dc() {
        return launch_dc;
    }

    public void setLaunch_dc(String launch_dc) {
        this.launch_dc = launch_dc;
    }
}
